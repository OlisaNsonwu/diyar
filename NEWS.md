...
-   Can now use multiple `case_lengths` or `recurrence_lengths` on the same event.
...
-   New function - `sub_criteria()`. A mechanism to supply subcriteria for a matching criteria in `links()`.
