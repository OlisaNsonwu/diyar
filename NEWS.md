...
-   Use more than one `case_length` or `recurrence_length` for the same event. Supply these as list.
    -   For each `case_length` and `recurrence_length`, use a different `overlap_method`. Supply these as list of matching lengths to `case_length` and `recurrence_length`.
