...
#' @rdname schema
#' @export
schema.pane <- function(x, int, separate, title = NULL, show_labels = c("window_label")) {
  panes <- x
  plt_df <- to_df(panes)
  ...
  #splits_windows <- splits_windows[1]
  ...
  f <- ggplot2::ggplot(data = plt_df) +
    ...
    ggplot2::geom_text(ggplot2::aes(x = (as.numeric(start) + as.numeric(end))/2, y= y2, colour = pane_n, label = win_l), data = border, nudge_y = .05, size = 5)
...
schema.epid <- function(x, date, episode_unit, case_length,
  ...
  if("length_arrow" %in% show_labels){
    f <- f + ggplot2::geom_segment(ggplot2::aes(x = start, y= y, xend = end, yend = mid_y_lead, linetype = wind_nm_l), color = "white", alpha= .9, data = case_l_ar[case_l_ar$nl_nm == "len" & !case_l_ar$no_ar & case_l_ar$wind_total > 1,], arrow = ggplot2::arrow(length = ggplot2::unit(scale_size(c(.5,.2), 500, plot_pts),"cm"), ends="last", type = "open"))
  }
  if("length_label" %in% show_labels){
    f <- f + ggplot2::geom_text(ggplot2::aes(x = (as.numeric(start) + as.numeric(end))/2, y= lab_y, label = nl_l), data = case_l_ar[case_l_ar$nl_nm == "len",], nudge_y = scale_size(c(.02, .06), 500, plot_pts), size = scale_size(c(2,5), 500, plot_pts), color = "white", alpha= .9, vjust = "bottom")
  }
  if(!isFALSE(show_labels)){
    f <- f +
