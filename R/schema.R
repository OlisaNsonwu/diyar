...
#'
#' @param x \code{[\link[=number_line-class]{number_line}|\link[=pid-class]{pid}|\link[=epid-class]{epid}|\link[=pane-class]{pane}]}
...
#' @details
#' The \code{schema} function creates a schema diagram for \code{\link[=number_line-class]{number_line}}, \code{\link[=pid-class]{pid}}, \code{\link[=epid-class]{epid}} and \code{\link[=pane-class]{pane}} objects.
...
#'
#' \emph{\code{\link[=number_line-class]{number_line}} objects are plotted as \code{\link[=epid-class]{epid}} objects.}
#'
#' @examples
#' schema(number_line(c(1, 2, 6, 8), c(5, 3, 8, 10)))
#'
...
schema.number_line <- function(x, show_labels = c("date", "case_overlap_methods"), ...){
  ...
  f <- schema(x, show_labels = show_labels, ...)
  return(f)
}
...
schema.epid <- function(x, title = NULL, show_labels = c("length_arrow"),
  ...
  if(any(!x@wind_id$wind_id1 %in% x@sn)){
    err <- x@.Data[!x@wind_id$wind_id1 %in% x@sn]
    err <- x[x@.Data %in% err]
    warning(paste0("Missing links in episode(s) ",
                   listr(err@.Data[!duplicated(err@.Data)], lim = 5),":\n",
                   "i - Is `x` a subset?\n",
                   "i - Please include all records in an episode\n",
                   "i - Episodes with missing links have been excluded.\n"),  call. = FALSE)
    x <- x[!x@.Data %in% err@.Data]
  }
  ...
  if(nrow(case_l_ar) > 0 & ("case_overlap_methods" %in% show_labels | "recurrence_overlap_methods" %in% show_labels)){
    ...
    plt_df$lead_wind_nm <- case_l_ar$wind_nm[rep_lgk]
    if("case_overlap_methods" %in% show_labels){
      rep_lgk <- which(plt_df$sn_lead != plt_df$sn & plt_df$lead_wind_nm == "Case length")
    ...
    if("recurrence_overlap_methods" %in% show_labels){
      rep_lgk <- which(plt_df$sn_lead != plt_df$sn & plt_df$lead_wind_nm == "Recurrence length")
      plt_df$overlap_method[rep_lgk] <- overlap_method(number_line(plt_df$start[rep_lgk],
                                                                   plt_df$end[rep_lgk]),
                                                       number_line(plt_df$lead_dt_a[rep_lgk],
                                                                   plt_df$lead_dt_z[rep_lgk]))
  ...
  if(!isFALSE(show_labels) | !is.null(custom_label)){
    if("case_overlap_methods" %in% show_labels | "recurrence_overlap_methods" %in% show_labels){
      f <- f + ggplot2::geom_text(ggplot2::aes(x = (as.numeric(.data$x_lead) + as.numeric(.data$mid_x))/2, y = (as.numeric(.data$y_lead) + as.numeric(.data$y))/2, colour = .data$epid, label = .data$overlap_method), nudge_y = scale_size(c(.01, .02), 500, plot_pts), size = scale_size(c(2,4), 500, plot_pts), vjust = "bottom", alpha = .7)
    }
...
schema.pid <- function(x, title = NULL, show_labels = TRUE,
  ...
  if(any(!x@link_id %in% x@sn)){
    err <- x@.Data[!x@link_id %in% x@sn]
    err <- x[x@.Data %in% err]
    warning(paste0("Missing links in record-group(s) ",
                   listr(err@.Data[!duplicated(err@.Data)], lim = 5),":\n",
                   "i - Is `x` a subset?\n",
                   "i - Please include all records in a record-group\n",
                   "i - Record-groups with missing links have been excluded.\n"),  call. = FALSE)
    x <- x[!x@.Data %in% err@.Data]
  }
  if(length(x) == 0) return(NULL)
