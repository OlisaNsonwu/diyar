...
#'
#' @param x \code{[\link[=pid-class]{pid}|\link[=epid-class]{epid}|\link[=pane-class]{pane}]}
...
schema.epid <- function(x, title = NULL, show_labels = c("length_arrow"),
  ...
  mths <- plt_df
  rep_lgk <- match(mths$sn_lead, case $pt_sn)
  mths$lead_dt_a <- mths$start[rep_lgk]
  mths$lead_dt_z <- mths$end[rep_lgk]
  mths <- mths[mths$sn_lead != mths$sn,]
  mths$overlap_method <- overlap_method(number_line(mths$start, mths$end),
                                        number_line(mths$lead_dt_a, mths$lead_dt_z))
