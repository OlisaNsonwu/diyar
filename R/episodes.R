...
episodes <- function(date, case_length = Inf, episode_type = "fixed", recurrence_length = case_length,
  ...
  while (max(web$repo$tag) == 20) {
    ...
    #
    # # Re-order batch tracked reference events
    # web$tmp$s_ord <- order(web$repo$assign_ord[web$tmp$tgt_pos])
    ...
    web$tmp2$s_ord <- order(
      web$tmp2$s_cri
      # web$repo$assign_ord[web$sys.tmp$ite_pos]
      # ,seq_len(length(web$sys.tmp$ite_pos))
    )
      # Window's reference record based on new selection
    ...
    if(FALSE){
      # Close records in current iteration but not included in record-pairs due
      # to `assing_ord`
      web$repo$tag[
        web$sys.tmp$ite_pos[!web$sys.tmp$ite_pos %in% web$rec.pairs$x_val]
      ] <- 10L
    }
    ...
    if(web$controls$use_recurrence){
      ...
      if(FALSE){
        ...
        web$rec.pairs$is_case_window[
          (!web$rec.pairs$nwEpi &
             ...
             is_even(web$repo$roll_n[web$rec.pairs$tr_pos]))
        ] <- 3
      }
      ...
      if(ite == 2){
        # browser()
        # as.data.frame(web$repo[c("pr_sn", "tag", "case_nm", "iteration", "ite.window")])
      }
      if(ite == 2){
        # browser()
      }
    ...
    for (w.code in seq_len(length(web$window_opts))) {
      ...
      if(w.name %in% c("recurrence", "case_for_recurrence")){
        ...
        if(all(web$sys.tmp$ite_tr_pos == 6)){
          # browser()
        }
    ...
    web$tmp$lgk2 <- web$repo$iteration[web$sys.tmp$ite_pos] == 0 &
      web$repo$tag[web$sys.tmp$ite_pos] != 20
    web$repo$iteration[web$sys.tmp$ite_pos[web$tmp$lgk2]] <- ite
  ...
  web$tmp$tgt_pos <- web$repo$pr_sn[web$repo$case_nm == -1]
  if(length(web$tmp$tgt_pos) > 0){
    web$tmp$tgt_pos <- index_multiples(
      web$tmp$tgt_pos,
      multiples = web$counts$dataset.n,
...
episodes_af_shift <- function(date, case_length = Inf, sn = NULL,
  ...
  web$repo$cumEnd <- as.numeric(web$repo$tmp.windowEnd)

  # web$repo$tmp.PrvWindowStart <- rev(rev(web$repo$date_a)[-1])
  lead.pos <- 2:length(web$repo$date_a)
  web$repo$tmp.NxtWindowStart <-
    web$repo$date_a[c(lead.pos, NA)]

  if(length(web$repo$strata) > 1){
    web$repo$strata.unq <- web$repo$strata[!duplicated(web$repo$strata)]
    web$repo$cumEnd <- sapply(web$repo$strata.unq, function(x){
      cummax(web$repo$cumEnd[web$repo$strata == x])
    }, simplify = TRUE)
    web$repo$cumEnd <- as.numeric(web$repo$cumEnd)
  }else{
    web$repo$cumEnd <- cummax(web$repo$cumEnd)
  }

  web$repo$change_lgk <- (web$repo$date_a > web$repo$cumEnd)
