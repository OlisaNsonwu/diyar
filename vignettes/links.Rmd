...
This function aims to provide a convenient and flexible approach to multistage deterministic linkage with mechanims to handle missing or incorrect data.
...
`sub_criteria` are required for user-defined logical tests however, it take more time to compare `sub_criteria` than it does  `criteria`. Therefore, you'll save some time if you only use a `sub_criteria` when the matching conditions can not be specified as a `criteria`. For example, the defined logical test `lgl_1` (above) was supplied as a `sub_criteria` however, `links()` will create the identifier faster if it's supplied as a `criteria`. See below.
