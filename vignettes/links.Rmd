...
Both strategies represent different outcomes. 
...
This is often faster but at the costs more in memory usage. With a large enough dataset, we will eventually run out of memory, especially when there is no `blocking_attribute`. Below we see how `link_records()` is used for the same fuzzy matching.
...
We can use `as.data.frame` or `as.list` to access the contents of each identifier inspect the difference.
```{r warning = FALSE}
