...
3. Episodes with specific events as their reference event (case). See ["Making a case definition"](#case_assignment)
...
Before we begin, let's go over some terms which will pop up now and again. An `event` is simply a dated record. A `period of event` or `event period` is a record with a start and end point in time. The "lengths" - `case_length` and `recurrence_length` are specific durations begining from an `event`. More specifically, the `case_length` is the duration begining with the `reference event` (first `event` of an `episode`) while `recurrence_length` is the duration begining from the other `events` subsequent to the case. 

All `events` within a `case_length` or `recurrence_length` form a `window`. A `window` equates to an `episode`. `events` outside a `window` belong to separate episode. What differentiates `rolling episodes` from `fixed episodes` is that when two or more `windows` overlap, they are `collpased` into a longer `window`. As a result, every `event` that belonged to the overlapping `windows` will now belong to this new and longer `window` (and by definition `episode`). Consequently, a `"fixed episode"` is never longer than its `case_length` while a `rolling episode` becomes longer than its `case_length` if the first `window` (also referred to `case window`) overlaps with another.
...
ds$f1 <- fixed_episodes(ds$date, case_length = 5, to_s4 = T, group_stats = T)
ds
```
...
```{r message=FALSE, warning=FALSE}
ds$r1 <- rolling_episodes(ds$date, case_length = 5, recurrence_length = 10, to_s4 = T, group_stats = T)
ds[c("f1","r1")]
```
...
In the `rolling episodes` example, records 1 to 3 are also assigned to one window (window ID `1`) however, unlike the `fixed episode` example, since records 4 and 5 both fall within 15 days (14-day difference) of the last `event` (record 3) of the previous `window` (`D-1`), they are both folded into `window` ID `1` which then becomes longer as a result. Since record 4 is the earliest of the two, it's flagged as the `recurrent (R) event` while record 5 is taken as a `duplicate`. 
...
df$r1 <- rolling_episodes(df$date, case_length = 14, recurrence_from_last = F, 
                          to_s4 = T, display = F)
df$r2 <- rolling_episodes(df$date, case_length = 14, recurrence_length = 16, to_s4 = T, display = F)
...
df$r4 <- rolling_episodes(df$date, case_length = 10, to_s4 = T, display = F)
...
df2$r1 <- rolling_episodes(df2$date, case_length = 3, to_s4 = T, display = F)
df2$r2 <- rolling_episodes(df2$date, case_length = 3, to_s4 = T, case_for_recurrence = T, display = F)
...
You can choose how many times to check for `episodes` using `episode_max`. When each `strata` of the dataset has the maximum number of episodes specified (`episode_max`), subsequent `events` given unique episode IDs, making them unique `cases`.
...
Similarly, you can choose how many `periods of recurrence` your case definition should permits i.e. how many times to check for `recurrent events`. You do this with `rolls_max`. The default is `Inf` i.e. check until there are no more `events` within the `recurrence period` of the `reference event`. See below.
...
Note that if `case_for_recurrence` is `TRUE`, after checking for `recurrence` in the last `recurrence period`, the function will still check for `duplicates` within the `case window` of the last `recurrent event`.
