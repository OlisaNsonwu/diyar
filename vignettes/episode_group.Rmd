...
Notice that when two periods/intervals overlap they are merged into one. This new period will envelope the two overlapped periods/intervals i.e. start of one and end of the other.

For a practical example, below are periods of hospital stay which are grouped into episodes - in this context defined as hospital spells. This is different from grouping the actual admission or discharge events.
...
Since periods can overlap in different ways, you can choose from a set of methods (`overlap_methods`) which should be considered in your analysis. The options available are; `"exact"`, `"across"`, `"inbetween"`, `"chain"`, `"aligns_start"` and `"aligns_end"`. The default option is to use all. Below are examples demonstrating each method.
...
# All results in the figure below  
```
...
epids_db$grp_l <- factor(epids_db$grp, levels = c("exact", "across","aligns_start","aligns_end","chain", "inbetween", "cb1", "cb2", "cb3"))
...
`case_length` in interval grouping is used in a similar manner. A negative `case_length` shortens the `"Case (C)"` interval before comparing it with other intervals. See results below (grouped episodes only).
...
epids_db2$grp_l <- factor(epids_db2$grp, levels = c("a","c","b"), labels = c("inbetween: 30-day episode length","inbetween: -1-day episode length","inbetween: -4-day episode length"))
