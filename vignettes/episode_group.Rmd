...
```{r message=FALSE, warning=FALSE}
ds$r1 <- rolling_episodes(ds$date, case_length = 5, recurrence_length = 10, to_s4 = T,
                          display = F)
ds
```
...
In the `fixed episodes` example, records 1 to 3 are assigned to a `window` - eventual episode ID `"E-1"` (Figure 2). In this window, record 1 is the `reference event` (`"(C)"`) and records 2 and 3 are `duplicates` (`"(D)"`) since they are within 15 days (difference of 14 days) of record 1. Record 4 is not within this 15-day period after record 3 and so begins a new window (and epsiode i.e. episode ID `"E-4"`) as its `reference event`. Record 5 is within 15 days of record 4 and so is assinged to the same window and episode (`"E-4"`). This process continues chronologically until all records have been assigned an episode ID.
...
Notice that all `events` have been assigned to the same `episode` in the examples above but the second one has 1 less `recurrent event`.
