...
#Introduction
In epidemiological analyses, an exact distinction between temporal events is not always possible. Therefore, static but reasonable cut-offs are used to distinguish between events. This is often an important aspect of a case definiton. For example, it's often important to distinguish between repeat positive infection results during a pathogen's incubation period. Another example, is distinguishing between infecions from a single pathogen and those by multiple pathogens within a certain period of time. 
...
+ episodes where each event occured at points in time e.g. medical diagnoses or traffic incidents
+ episodes where each event occured over a period of time e.g. hospital admission or incubation periods. See [interval grouping](#interval_grouping)
+ episodes where the events are known to reoccur
+ episodes where certain type events are taken as index (reference) events. See ["user defined case assignment"](#custom_sort)
...
## Overview
An episode as produced by `fixed_episodes()`, `rolling_episodes()` or `episode_group()` is a set events (dated records) within a specific period. Each episode begins with an index event and may contain duplicate or recurrent events. 
...
+ `event period` - `events` with a known duration i.e. start and end time points
+ `reference event`/`reference event period` - index `event` or `event period`
+ The "lengths" -  cut-off for distinguishing event
    + `case_length` - cut-off period distinguishing an `event` from another
    + `recurrence_length` - cut-off peroid distinguishing an `event` from a recurrence of the same `event`
...
+ `episode` - all `event` within the same cut-offs - `case_length` and `recurrence_length`. Every overlapping `window` forms an `episode`.
...
In `fixed_episodes()`, a `window` always equates to an `episode`. `events` from a different `window` belongs to a separate `episode`. However with `rolling_episodes()`, if two or more `windows` overlap, they are collapsed into a larger `window`, indicating that the `episode` is still on-going or has reocurred. As a result, every `event` belonging to both `windows` are now considered part of the same `"rolling episode"`.

## Making case definitions
### epid objects
Firstly, let's go through what `episodes` look like. They are stored as `epid` objects (`S4` object class). An `epid` object is an `episode` identifier with `slots` for useful information for each `episode`. See `?fixed_episodes` for the details of each slots. However, for those in the `format.epid`, see below.
...
dplyr::tibble(events =ds, epids= ep)
```
...
*to_df() is a convienient accessor function for every slot in an `epid` object - to_df(epids)*

### Cut-offs - case and recurrence lengths 
Your first consideration should be a cut-off point to distinguish `episodes` - `case_length`. A `case_length` of `5` means that; repeat `events` within `6 days (Time difference of 5 days)` of the `reference event` are considered `duplicates` of the `reference event`. See below.
...
Alternatively, your cut-off can be a specific range after the `reference event`. For example, a `case_length` of ``r format(number_line(12, 16))`` means that; repeat `events` within `13` and `17` days after the `reference event` are considered `duplicates` of the `reference event`. 

*Ranges are created by number_line(). See accompanying [vignette](number_line.html).*
...
```{r echo=FALSE, fig.height=4, fig.width=8.5, message=FALSE, warning=FALSE}
plot_epid(epid = ds$f2, date= ds$date, case_length = 5)
```

By default, `events` before the `13th` day are skipped. You can change `skip_if_b4_lengths` to `FALSE`, so that these are not skipped.
...
```{r echo=FALSE, fig.height=4, fig.width=8.5, message=FALSE, warning=FALSE}
plot_epid(epid = ds$f3, date= ds$date, case_length = 5)
```

### Reference events
Your next consideration should be which `events` will be the index `event` (`reference events`). It's common pratcice for the earliest `event` to be taken as the `reference event`. However, there are times when the most recent `event` will be more relevant. For example, if a deceased patient died after a series of positive infeciton results, the most recent result could be considered more relevant and so taken as the `reference event`.

The first approach is the default behaviour for `fixed_epsiodes()` and `rolling_episodes()`. However, by changing `from_last` to `TRUE`, `episode tracking` will begin at the most recent `event` and proceeed backwards in time, making that the `reference event`.
...
There can be situations when the type of `event` is more relevant in determining an index `event` rather than its chronological order. For example, when tracking patients with positive infection results for; human immunodeficiency virus (HIV), hepatitis B virus (HBV) and *Mycobacterium tuberculosis* (MyCt), you may want to track concurrent infections begining or ending with the HIV diagnosis. The example below shows how to handle situations like this.
...
# Track concurrent infections up to 6 months AFTER the HIV diagnosis
...
# Using `data_source` populates the `epid_dataset` slot. It's useful when you're working with different datasets
...
Notice that in `ep1`, the HIV diagnosis and the HBV diagnosis before it are not grouped into the same `episode` despite being within 6 months of each other. This also happens with the HIV and MyCt diagnosis in `ep2`. 

To remedy the situation above, you can track `events` that occur before or after an index `event`. This is either done by changing `bi_direction` to `TRUE` or by using an applicable cut-off range i.e. `r format(number_line(-6,6))`. See the examples below. 

```{r warning=FALSE}
# Track preceeding and concurrent infections up to 6 months BEFORE OR AFTER the HIV diagnosis

# method 1a
# Track diagnoses 6 months AFTER the HIV diag while `bi_direction` tells the function to also check 6 months BEFORE the HIV diagnosis
ds2$ep3a <- fixed_episodes(date = ds2$date, custom_sort = ds2$user_ord, case_length = 6, 
                           bi_direction = T, episode_unit = "months", display = F)

# method 1b
# Track diagnoses 6 months BEFORE the HIV diag while `bi_direction` tells the function to also check 6 months AFTER the HIV diag.
...
# method 2a
# Track diagnoses 6 months BEFORE or AFTER the HIV diag with an applicable. There's no need for `bi_direction`.
rng <- number_line(-6, 6)
...
# method 2b
# Track diagnoses between 2 and 6 months AFTER the HIV diag while `bi_direction` tells the function to also check between 2 and 6 months BEFORE the HIV diag.
# Diagnoses before 2 months on either side are skipped as mentioned in the "lengths" section
rng2 <- number_line(2, 6)
ds2$ep5 <- fixed_episodes(date = ds2$date, custom_sort = ds2$user_ord, case_length = rng2, bi_direction =T, episode_unit = "months", 
                          display = F)
...
When different `events` have the same level of preference - `custom_sort` values, their chronological order is used to break ties. This will be the earliest or most recent `event` depending on `from_last`. Alternatively, you can break such ties with additional levels of `custom_sort`.
...
Another important consideration for a case definition should be if the `events` reoccur. For example, if a subsequent "relapse" of the same disease occurs aftera  sucessful treatment therapy, it may be more relevant to treat this as the same episode of the initial `event` using `rolling_episodes()`.

*fixed_episodes() will treat this as a separate `event`* 

All arguments in `fixed_episodes()` (including  `case_length`) function the same way in `rolling_episodes()`. However, the `recurrence_length` argument, which is unique to `rolling_episodes(),` provides the cut-off period for `recurrent events`. For example, a `recurrence_length` of `10` means that; repeat `events` within `10` days of the last `duplicate event` (of the initial index `event`) are considered a recurrence of the index `event`.

*If recurrence_length is not supplied, it's assumed to be the same as `case_length`.*

**Figure 1b: rolling episodes**
```{r warning=FALSE}
...
Similarly, for `recurrence_length`, your cut-off can be a range. For example, a `recurrence_length` of ``r format(number_line(13, 15))`` means that; repeat `events` within `14` and `16` days after the last `duplicate event` are considered a recurrence of the `reference event`. *You can also change `skip_if_b4_lengths` to `FALSE`, so that these are not skipped.* 
...
```{r echo=FALSE, fig.height=4, fig.width=8.5, message=FALSE, warning=FALSE}
plot_epid(epid = ds$r2, date= ds$date, case_length = 5, recurrence_length = 10)
```

#### Index events for periods of recurrence

Unlike `"fixed episodes"`, each `"rolling episode"` has at least two index `events` - one for the whole `episode` and one for each period of recurrence. By default, the last `event` of the previous window is the index `event` from which the next period of recurrence is calculated. However, you can use change `recurrence_from_last` to `FALSE`, so that the first `event` in the previous window is used instead. See a demonstration of this below.
...
Notice that in the examples, all `events` have been assigned to the same `episode` but the number of `recurrent events` differ.

### Number of recurrence periods and episodes
Some case definitions may only require you to track the first `n`-number of episodes or that `events` recurring after `n`-number of times should be considered a separate episode. You can incorporate these requirements using `episodes_max` and `rolls_max` (only applicable to `rolling_episodes()`). When each `strata` (see stratified grouping) of the dataset has the maximum number of `episodes` (`episodes_max`), subsequent `events` are skipped and assigned unique episode IDs. The default option for `episodes_max` is `Inf` i.e. continue until every `event` has been grouped into `episodes`.
...
During `episode grouping`, when `rolls_max` is reached - maximum number of times an `event` can reocur, the `episode` ends. A new one only begins if the number of `episodes` in that `strata` is less than `episode_max`. The default option for `rolls_max` is `Inf` i.e. check until there are no more `events` within the `recurrence period` of the `reference event`. See below.
...
### Interval grouping
Some case definitions can involve `events` that occur over a period of time as opposed to single time points e.g hospital admissions. `fixed_episodes()` and `rolling_episodes()` can be used to track such `event periods` into `episodes`. This type of interval grouping is recommended when the end point of an event is known. For example, it's near impossible to know the exact point in time an individual diagnosed with the common cold recovers however, it's a reasonable assumption that a second diagnosis after 3 months is not the same one that was diagnosed initially one. For situations like this, the recommend approach is the methods we've covered so far. Conversely, hospital admissions typically have recorded admission and discharge times or dates and so interval grouping is recommended. This being said, in several situations they can be used interchangeably. See below. 
...
# Track fixed episodes from events using a 5 day cut-off 
...
In interval grouping, when `case_length` is `0`, `event periods` that overlap with the index `event peroid` are grouped one `episode`. However, if when `case_length` is `> 0`, those `event periods` within `n`-days (or any time unit) after the `end_point()` of the index `event period` are also grouped into one the same `episode`. See below. 
...
**Figure 8b: Tracking the periods of stay (admission -> discharge)**
```{r message=FALSE, warning=FALSE}
...
A positive "length" means that; `event periods` overlaping with the index `event period` and those within `n`-days after the end_point() of the index `event period` will be grouped into the same episode. See an example below.
...
A negative "length" means that; only `event periods` within `n`-days before the end_point() of the index `event period` will be grouped into the same `episode`. See the examples below.
...
In practice, you'll want to track episodes separately for different `stratas` such as patients, pathogens, sources of infection or any combination of these i.e. `episodes` from one patient will be different from other patients, or `episodes` of HIV diagnoses will be different those of HBV diagnoses. 

*`episodes` from different `strata` can have a different `case_lengths` and/or `recurrence_lengths`.*

*`record_group()` is useful for creating a group identifier which can then be used as a `strata`. See the [record group vignette](record_group.html) for further details.*
...
By adding `infection` to the `strata` argument, `"UTI"` and `"BSI"` diagnoses will not be in the same `episode`. Furthermore, the `case_length` and `recurrence_length` for each will be different as specified in the `epi` and `recur` columns respectively.
...
#### Sub-strata
Sub-stratas are when `events` within a `strata` have different cut-offs. The case definition below demonstrates how this maybe beneficial.
...
In this example, whether or not the infection is treated should be viewed as the sub-strata, while the the source of infection taken as the `strata`.
...
`fixed_episodes()` and `rolling_episodes()` both run of `episode_group()`, and that in turn is essentially an endless loop that only stops when every record has been grouped into `episodes`. However, for some case definitions, you may already know the kinds of `episodes` that you are not of interest in. There are 3 mechanisms to skip such `events`/`episodes`, leading to a reduced processing time. Skipped `events` are assigned unique episode IDs and their `case_nm` is flagged as `"Skipped"`.

### Skip `stratas` of the dataset 
...
*changing display to TRUE, shows the progress of the grouping process and shows how many iterations you've taken to complete the entire process.*
...
`skip_order` is related to the `custom_sort` order. A `skip_order` of `1` means that; once every `event` with your first level of your `custom_sort` has been grouped into `episodes`. The tracking process ends and every other `event` is skipped.

This comes in handy when you're grouping rare `events` with common `events`, the rare `events` are the index `events` and it's a hassle to subset the dataset. Since you're only interested in `episodes` where rare `events` are the index `events`, the rest can be skipped. 
...
# Breakup episodes without events from the `DS3` `data_source` - must be linked to other `data_sources`  
dbg$ep_b <- episode_group(dbg, date = date, case_length = case_len, episode_type ="rolling", 
                          recurrence_length = recurrence, data_source = dataset, to_s4 = T,
                          data_links = "DS3", display = F)

# Breakup episodes without events from the `DS3` `data_source` - does not have to be linked to other `data_sources`   
dbg$ep_c <- episode_group(dbg, date = date, case_length = case_len, episode_type ="rolling", 
                          recurrence_length = recurrence, data_source = dataset, to_s4 = T,
                          data_links = list(g="DS3"), display = F)

# Breakup episodes without events from the `DS3` `data_source` - must be linked to record from `DS1`
...
Note that since `reference events` are not always known ahead of time, `episodes`  will be grouped first. Afterwards, `episodes` without the required `data_links` are then unlinked and shown as `"Skipped"`. However, if an entire `strata` does not have the required `data_links`, that `strata` is skipped to save time.
...
In the examples above, episode grouping was done mostly by days (`episode_unit`). However, it can be done by other units of time e.g. hours, days or weeks. Acceptable options are "seconds", "minutes", "hours", "days", "weeks", "months" or "years". Below is an example of `episode grouping` by the hour.
...
This package should prove useful in epidemiological analyses however, it's also applicable to analyses in any field where the goal is to group events into temporal groups.
