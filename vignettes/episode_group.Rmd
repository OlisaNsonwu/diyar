...
plot <- plot %>%
  ...
  mutate(
    ...
    lab =ifelse(case_nm=="Case" & var=="discharge_dt", case_nm, ""), 
    method = factor(method, levels = c("overlap","aligns_start","aligns_end","chain", "within", "chain and within"))
    )
...
As with the last example above, you can use different combinations of each `overlap_method` option. More example below; 
...
You can also have rolling episodes of intervals.
...
`"fixed"` episode grouping with a sufficiently long `case_length` can give the same results as `"rolling"` episode grouping with a short `case_length` but sufficiently long `recurrence_length`. However, it's important that they are not the same process despite similar results. See below.
