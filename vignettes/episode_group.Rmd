...
+ with periods of reccurence (`recurrence_length`)
...
In the `fixed episodes` example, records 1 to 3 are assigned to a `window` (i.e. window ID `1`). In this window, record 1 is the `reference event` (case) and records 2 and 3 are the  `duplicates`, because they are within 15 days (14-day difference) of record 1. This `window` then becomes one `episode` (episode ID `1`; Figure 2a). Record 4 is not within this 15-day period after record 3 and so begins a new `window` (and `episode` i.e. window and episode ID `4`). Record 5 is within 15 days of record 4 and so is assinged to the same window and episode (window and episode ID `4`). This process continues chronologically until all records have been assigned to an `episode`.
...
**Figure 6b: Maximum of one rolling episode grouping per strata**
```{r message=FALSE, warning=FALSE}
dbs$eps_2 <- rolling_episodes(strata = dbs$pid, date = dbs$date, case_length = 3, display = FALSE, episodes_max = 1)
```

```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
plot_epid(epid = dbs$eps_1, date = dbs$date, case_length =3, recurrence_length = 3)
```
...
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
plot_epid(epid = dbs$backward, date = dbs$date, case_length = 14)
```
...
**Figure 9a: Grouped episodes if considering only "exact" overlap method**
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
eps <- epids[["exact"]]
plot_epid(epid = eps, date = hos_admin$admin_period[eps@sn] , case_length = 0)
```

**Figure 9b: Grouped episodes if considering only "chain" overlap method**
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
eps <- epids[["chain"]]
plot_epid(epid = eps, date = hos_admin$admin_period[eps@sn] , case_length = 0)
```
**Figure 9c: Grouped episodes if considering "chain" or "inbetween" overlap method**
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
eps <- epids[["cb1"]]
plot_epid(epid = eps, date = hos_admin$admin_period[eps@sn] , case_length = 0)
```
...
+ Using positive "lengths" will collapse two periods into one, if the second one is within (or any other method of `overlap`) the first one or `n`-days (or any other `episode_unit`) after the `end_point()` of the first period. 
+ Using negative "lengths" will collapse two periods into one, if the second one is within (or any other method of `overlap`) the `start_point()` and `n`-days (or any other `episode_unit`) before the `end_point()` of the first.
+ Using negative "lengths" that's larger than the `number_line_width()` will collapse two periods into one, if the second one is within (or any other method of `overlap`) `(n - number_line_width())`-days (or any other `episode_unit`) before the `start_point()` and the `start_point()`of the first.
    + **note that this will happen even if `from_last` is TRUE**

*Examples below*

```{r warning=FALSE}
pa <- c("01/04/2019", "03/04/2019", "07/04/2019","12/04/2019")
pz <- c("10/04/2019", "05/04/2019", "09/04/2019","13/04/2019")
...
eps_a <- eps_a[eps_a@epid_total>1]

eps_a
```

```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
plot_epid(epid = eps_a, date = periods[eps_a@sn], case_length = 6)
```

**Figure 9e: Fixed episodes with a case_length of -2**
```{r message=FALSE, warning=FALSE}
eps_b <- fixed_episodes(periods, case_length =-2, group_stats = T, display = F)
eps_b <- eps_b[eps_b@epid_total>1]

eps_b
```

```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
plot_epid(epid = eps_b, date = periods[eps_b@sn], case_length = -2)
```

**Figure 9f: Fixed episodes with a case_length of -6**
```{r message=FALSE, warning=FALSE}
eps_c <- fixed_episodes(periods, case_length =-6, group_stats = T, display = F)
eps_c <- eps_c[eps_c@epid_total>1]

eps_c
```

```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
plot_epid(epid = eps_c, date = periods[eps_c@sn], case_length = -6)
```
