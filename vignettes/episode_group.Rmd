...
#Introduction
In epidemiological analyses, an exact distinction between temporal events is not always possible. Therefore, static but reasonable cutoffs are used to distinguish between events. This is an important aspect of a case definition. For example, it's often important to distinguish between repeat positive infection results during a pathogen's incubation period. Another example, is distinguishing between infections from a single pathogen and those by multiple pathogens within a certain period in time. 
...
+ `episode` - all `event` within the same cutoffs - `case_length` and `recurrence_length`. Every overlapping `window` forms an `episode`
...
*to_df() is a convenient accessor function for every slot in an `epid` object - to_df(`epids`)*

### cutoffs - case and recurrence lengths 
Your first consideration should be a cut-off point to distinguish between `episodes` - `case_length`. A `case_length` of `5` means that; repeat `events` within `6 days (Time difference of 5 days)` of the `reference event` are considered `duplicates` of the `reference event`. See below.
...
### Reference events
Your next consideration should be which `events` will be the index `event` (`reference events`). It's common practice for the earliest `event` to be taken as the `reference event`. However, there are times when the most recent `event` will be more relevant. For example, if a deceased patient died after a series of positive infection results, the most recent result could be considered more relevant and so taken as the `reference event`.
...
Another important consideration for a case definition should be if the `events` recur. For example, if a relapse of the same disease occurs after a  successful treatment therapy, it may be more relevant to treat this as the same `episode` of the initial `event` using `rolling_episodes()`.
...
`rolls_max` is reached - maximum number of times an `event` can recur. When `rolls_max` is reached, the `episode` ends. A new one only begins if the number of `episodes` in that `strata` is less than `episode_max`. The default option for `rolls_max` is `Inf` i.e. check until there are no more `events` within the `recurrence period` of the `reference event`. See the examples below.
...
In interval grouping, when `case_length` is `0`, `event periods` that overlap with the index `event period` are grouped together as one `episode`. However, if `case_length` is `> 0`, those `event periods` within `n`-days (or any time unit) after the `end_point()` of the index `event period` are also grouped together as the same `episode`. See below. 
...
**Figure 7c: Tracking the periods of continuous hospital stay i.e. re-admission within 15 days**
```{r message=FALSE, warning=FALSE}
...
However, a negative "length" means that; only `event periods` within `n`-days BEFORE the   `end_point()` of the index `event period` will be grouped into the same `episode`. See the examples below.
...
**Figure 9c: Fixed episodes with a case_length of -6**
```{r message=FALSE, warning=FALSE}
...
In practice, you'll want to track episodes separately for different `strata` such as patients, pathogens, sources of infection or any combination of these. This means that `episodes` from one patient will be different from other patients, or `episodes` of HIV diagnoses will be different those of HBV diagnoses. 
...
#### Sub-strata
Sub-strata are when `events` within the same `strata` have different cutoffs. The case definition below demonstrates how this could be beneficial.
...
In this example, whether or not the infection is treated should be viewed as the sub-strata i.e. within the same patient (`strata`), an `episode` is expected to last longer if it's not treated. 
...
- Unless required, `case_length` and `recurrence_length` should be consistent in each `strata` otherwise, you'll inadvertently create a sub-strata
...
Some case definitions are so specific that only certain kinds of `episodes` will be required. In such cases, you can save save time by skipping `events` from `episodes` that aren't required. There are 3 mechanisms to skip such `events`. 
...
In dataset with multiple levels of `custom_sort`, you can skip `episodes` that will begin with the `nth` order (`skip_order`) of `custom_sort`. A `skip_order` of `1` means that; after every `event` with the first level of your `custom_sort` has been grouped into `episodes`, the tracking process comes to an end.
...
# Breakup episodes without events from the `DS3` `data_source` - the `DS3` event MUST BE linked to events from other `data_sources`  
dbg$ep_b <- episode_group(dbg, date = date, case_length = case_len, episode_type ="rolling", 
                          recurrence_length = recurrence, data_source = dataset, to_s4 = T,
                          data_links = "DS3", display = F)

# Breakup episodes without events from the `DS3` `data_source` - the `DS3` event DOESN'T HAVE to be linked to events from other `data_sources`   
dbg$ep_c <- episode_group(dbg, date = date, case_length = case_len, episode_type ="rolling", 
                          recurrence_length = recurrence, data_source = dataset, to_s4 = T,
                          data_links = list(g="DS3"), display = F)

# Breakup episodes without events from the `DS3` `data_source` - the `DS3` event MUST BE linkned to events from the `DS1` `data_source`
...
5. Change `from_last` to `TRUE` if you want to start episode grouping at the most recent record thereby making it the `"case"`. Note that this is not the same as starting episode grouping at the earliest record (`from_last` is `FALSE`) and then picking the most recent record in that episode as the `"case"`
...
These functions should prove useful in epidemiological analyses however, it's also applicable to analyses in any field where the goal is to group events into temporal groups.
