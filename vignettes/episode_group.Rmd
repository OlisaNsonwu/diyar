...
```{r include=FALSE}
epid_plots <- function(df1){
  df1$epid <- as.character(df1$epid)
  df1$wind_id_l <- paste0("W",as.character(df1$wind_id))

  # Events
  pts <- df1[c("date", "epid", "wind_id","case_nm","wind_id_l","sn","wind_nm")]
  pts$case_nm <- paste(pts$case_nm,"\n",ifelse(pts$sn %in% unique(pts$wind_id), "(reference)", ""))

  # Episode and window labels
  pts$y <- 1
  pts$e_lab <- ifelse(!duplicated(pts$epid, fromLast = T), paste0("Episode ", as.numeric(as.factor(pts$epid)), ""),"")
  pts$w_lab <- ifelse(!duplicated(pts$wind_id, fromLast = T), paste0("Window ", as.numeric(as.factor(pts$wind_id_l)), "\n(", tolower(pts$wind_nm), "\nwindow)"), "")
  pts$y2 <- 0.81
  pts$y3 <- 0.64

  # case length
  cl <- df1[df1$sn %in% unique(df1$wind_id[df1$wind_nm=="Case"]), c("date", "epid", "c", "wind_id_l")]
  cl$end_dt <- cl$date + cl$c
  cl$y <- 1.1 - (1:nrow(cl) * 0.01)
  cl <- gather(cl, vr, date, -c(epid, c, y, wind_id_l))
  cl$lab <- ifelse(cl$vr=="date", paste0("Case length (",cl$c,"-day\ndifference)"), "")

  # recurrence lengths
  rl <- df1[df1$sn %in% unique(df1$wind_id[df1$wind_nm!="Case"]), c("date", "epid", "r","wind_id_l")]

  rl$end_dt <- rl$date + rl$r
  rl$y <- 0.94 - (1:nrow(rl) * 0.01)
  rl <- gather(rl, vr, date, -c(epid, wind_id_l, r, y))
  rl$lab <- ifelse(rl$vr=="date", paste0("Recurrence\nlength (",rl$r,"-day\ndifference)"), "")

  # windows
  windows <- pts[grepl("Case", pts$case_nm) | !duplicated(pts$wind_id, fromLast = T),]
  # episodes
  episodes <- pts[grepl("Case", pts$case_nm) | !duplicated(pts$epid, fromLast = T),]
  ...
  ggplot(data= pts) +
    ...
    geom_line(data=episodes, aes(x=date, y=y3, group=epid, colour =epid), arrow = arrow(length=unit(0.1,"cm"), ends="both", type = "closed")) +
    geom_text(data=episodes, aes(x=date, y=y3, group=epid, colour =epid, label=e_lab, hjust="left"), nudge_y = -.03, nudge_x = -5, size=3) +

    geom_line(data=windows, aes(x=date, y=y2, group=epid, colour =wind_id_l), arrow = arrow(length=unit(0.1,"cm"), ends="both", type = "closed")) +
    geom_text(data=windows, aes(x=date, y=y2, group=wind_id_l, colour =epid, label=w_lab, hjust="left"), nudge_y = -.06, nudge_x = -1.6, size=3) +

    geom_line(data=cl, aes(x=date, y=y, group=wind_id_l, colour =epid), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed")) +
    geom_text(data=cl, aes(x=date, y=y, group=epid, colour =epid, label=lab, hjust="left"),  nudge_y = .04, size=3) +

    geom_line(data=rl, aes(x=date, y=y, group=wind_id_l, colour =epid), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed"), linetype=2) +
    geom_text(data=rl, aes(x=date, y=y, group=epid, colour =epid, label=lab, hjust="left"),  nudge_y = -.04, size=3) +

    #scale_x_date("April", date_breaks = "1 day", date_labels = "%d", limits = c(dmy("01/04/2019"), dmy("20/04/2019") )) +
    scale_x_date("April", date_breaks = "1 day", date_labels = "%d") +
    #lims(y=c(0.55,1.2)) +
    theme(
...
+ with defined periods of reccurence (`recurrence_length`)
...
Before we begin, let's go over some terms which will pop up now and again. An `event` is simply a dated record. A `period of event` or `event period` is a record with a start and end point in time. The "lengths" - `case_length` and `recurrence_length` are specific durations after an `event`. More specifically, the `case_length` the a specified duration after the `reference event` (first `event` of an `episode`) while `recurrence_length` is a specified duration after other `events` subsequent to the `reference event`. 

All `events` within the same `case_length` or `recurrence_length` form a `window`. A `window` equates to an `episode`. `events` outside a `window` belong to separate `episodes`. What differentiates `rolling episodes` from `fixed episodes` is that when two or more `windows` overlap, they are `collpased` into a larger `window`. As a result, every `event` that belonged to the overlapping `windows` will now belong to this new and larger `window` (and by definition `episode`). Consequently, a `"fixed episode"` is never larger than its `case_length` while a `rolling episode` becomes larger than its `case_length` if the first `window` (also referred to `case window`) overlaps with another.
...
ds$f1 <- fixed_episodes(ds$date, case_length = 5, group_stats = T)
ds
```
...
pts <- df[c("date", "epid", "wind_id", "case_nm")]
pts$y <- 1
pts$e_lab <- ifelse(pts$case_nm=="Case", paste0("Episode ", as.numeric(as.factor(pts$epid)), ""),"")
pts$w_lab <- ifelse(pts$case_nm=="Case", paste0("Window ", as.numeric(as.factor(pts$window)), " (case window)"),"")
pts$y2 <- 0.9
pts$y3 <- 0.8
...
```{r message=FALSE, warning=FALSE}
ds$r1 <- rolling_episodes(ds$date, case_length = 5, recurrence_length = 10, group_stats = T)
ds[c("f1","r1")]
```
...
df$epid <- as.character(df$epid)
df$wind_id <- paste0("W",as.character(df$wind_id))

pts <- df[c("date", "epid", "wind_id")]
pts$case_nm <- c("Case\n(Reference)", "Duplicate\n(Reference) Event", "Recurrent\nEvent","Duplicate\nEvent")
pts$y <- 1
pts$e_lab <- ifelse(!duplicated(pts$epid, fromLast = T), paste0("Episode ", as.numeric(as.factor(pts$epid)), ""),"")
pts$w_lab <- ifelse(!duplicated(pts$wind_id, fromLast = T), paste0("Window ", as.numeric(as.factor(pts$wind_id)), "\n", c("(case window", rep("(recurrence window)",3))),"")
pts$y2 <- 0.7
pts$y3 <- 0.6
...
rl <- df[2:3, c("date", "epid", "r", "wind_id")] 
rl$end_dt <- rl$date + c(10,3)
rl <- gather(rl, vr, date, -c(epid, wind_id, r))  
rl$y <- 0.8
...
ggplot(data= pts) +
  ...
  geom_line(aes(x=date, y=y2, group=epid, colour =wind_id), arrow = arrow(length=unit(0.1,"cm"), ends="both", type = "closed")) +
  geom_text(aes(x=date, y=y2, group=wind_id, colour =epid, label=w_lab, hjust="left"), nudge_y = -.03, nudge_x = -1.6, size=3) +
  geom_line(data=cl, aes(x=date, y=y, group=epid, colour =epid), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed")) +
  geom_text(data=cl, aes(x=date, y=y, group=epid, colour =epid, label=lab, hjust="left"),  nudge_y = .04, size=3) +
  
  geom_line(data=rl, aes(x=date, y=y, group=wind_id, colour =epid), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed"), linetype=2) +
  geom_text(data=rl, aes(x=date, y=y, group=epid, colour =epid, label=lab, hjust="left"),  nudge_y = .04, size=3) +
...
In the `fixed episodes` example, records 1 to 3 are assigned to a `window` (i.e. window ID `1`). In this window, record 1 is the `reference event` (case) and records 2 and 3 are the  `duplicates`, because they are within 15 days (14-day difference) of record 1. This `window` then becomes one `episode` (episode ID `1`, Figure 2). Record 4 is not within this 15-day period after record 3 and so begins a new `window` (and `episode` i.e. window and episode ID `4`). Record 5 is within 15 days of record 4 and so is assinged to the same window and episode (window and episode ID `4`). This process continues chronologically until all records have been assigned to an `episode`.

In the `rolling episodes` example, records 1 to 3 are also assigned to one window (window ID `1`) however, unlike the `fixed episode` example, since records 4 and 5 both fall within 15 days (14-day difference) of the last `event` of the previous `window` (record 3), they are both folded into `window` ID `1` which then becomes larger as a result. Since record 4 is the earliest of the two, it's flagged as the `recurrent event` while record 5 is taken as a `duplicate`. 
...
**Figure XX: recurrence_from_last == TRUE (default)**
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
dfp <- cbind(df, to_df(df$r2))
dfp$r <- 16
dfp$c <- 14
...
# Example 2
df2 <- data.frame(
  date = seq.Date(as.Date("01/04/2019", "%d/%m/%Y"), as.Date("21/04/2019","%d/%m/%Y"), by="3 day")
)

df2$sn <- 1:nrow(df2)

df2$r1 <- rolling_episodes(df2$date, case_length = 6, recurrence_length = 4, display = F, sn=df2$sn)
df2$r2 <- rolling_episodes(df2$date, case_length = 6, recurrence_length = 4, case_for_recurrence = T, display = F, sn=df2$sn)

df2
```

**Figure XX: case_for_recurrence == FALSE (default)**
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
dfp <- cbind(df2, to_df(df2$r1))
dfp$r <- 4
dfp$c <- 6
...
**Figure XX: case_for_recurrence == TRUE **
```{r echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
dfp <- cbind(df2, to_df(df2$r2))
dfp$r <- 4
dfp$c <- 6
...
dbs$eps_1 <- fixed_episodes(strata = dbs$pid, date = dbs$date, case_length = 3, display = FALSE, episodes_max = 1)

# Maximum of one rolling episode grouping per strata
dbs$eps_2 <- rolling_episodes(strata = dbs$pid, date = dbs$date, case_length = 3, display = FALSE, episodes_max = 1)

# Maximum of two fixed episodes grouping per strata
dbs$eps_3 <- fixed_episodes(strata = dbs$pid, date = dbs$date, case_length = 3, display = FALSE, episodes_max = 2)
...
Note that if `case_for_recurrence` is `TRUE`, after checking for `recurrence` in the last `recurrence period`, the function will also check for the for `duplicates` within the `case window` of the last `recurrent event`.
...
fixed_episodes(date=dates, case_length=6, display=FALSE, group_stats = TRUE)

# user defined preference via custom sort is prioritised before from_last 
fixed_episodes(date=dates, case_length=6, custom_sort = user_sort, display=FALSE, group_stats = TRUE)

# user defined preference via custom sort is prioritised before from_last. 
# Duplicates are flagged from both directions
fixed_episodes(date=dates, case_length=6, custom_sort = user_sort, display=FALSE, 
...
# Each unit is relative to a set number of seconds shown below. 
diyar::episode_unit
...
For example, with the `hourly_data` dataset, you can decide to exclude `"GP1"` and `"GP2"` records from episode grouping as shown below. Records excluded this way are assigned unique episode IDs.
...
6. If you require the `"case"` to be the earliest or most recent event or a particular type of event, use `custom_sort` in combination with `from_last`. If not, ignore this argument. See [user defined case assignment](#custom_sort)
