...
+ Using negative "lengths" that's larger than the `number_line_width()` will collapse two periods into one, if the second one is within (or any other method of `overlap`) `(n + 1 + number_line_width())`-days (or any other `episode_unit`) before the `start_point()` and the `start_point()`of the first.
