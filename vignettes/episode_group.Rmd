...
## Skipping records to save time

At its core `episode_grouping()` is an endless loop that stops when every record has been grouped into episodes based on your case definition. However, certain kinds of episode may not be required, so you can reduce the overall processing time by skipping these. The 3 main ways of doing this are explains below. `display` shows you how many iterations of the process you've gone through.

### Skip `strata(s)` of the dataset 
You can skip an entire `strata` of the dataset by changing it to `NA`. In the example below, we've decided to skip `"GP1"` and `"GP2"` records to save time since. Skipped records are assigned unique episodes IDs and their `case_nm` is flagged with `"Skipped"`.
...
`custom_sort` gives a preferential order for case assignment. The means that in a `custom_sort` order of `1-3`, every record with a value of `1` must have been grouped before any with a value of `2` or `3` can be taken as a `reference event` i.e. preference for selecing `reference events` (case assignment) is `1 > 2 > 3`. You can decide to stop at any point in this order using `skip_order`. This comes in handy when you're grouping rare `event(s)` with common `events`, and your `reference events` are the rare ones. If you're only interested in `episodes` where rare `event(s)` are the `reference events` then you can skip the rest to the rest. 

In the example below, the `custom_sort` order is `"UTI" > "BSI" > "RTI"`. This is supplied to the function as `c_sort`. Notice that in skipped example `ep_a`, there are no `episodes` that begin (`(C)`) with a `"UTI"` or `"RTI"` record.
...
The second method above is only useful if you know or have a preference for `reference events`. Conversely, this method allows you skip entire `episodes` that won't contain particular records regardless of whether they are `reference events` or not. 
