...
# Oveview
`diyar` is a data analytical package for linking entities (records) with shared characteristics.
Linked entities are assigned to unique groups with identifiers.
...
These functions very flexible in terms of what and how comparisons are done, as well as what are considered matches.
Although each is best suited to certain use cases, their functionalities can sometimes overlap albeit less efficiently.
...
This is demonstrated below with the 2-setp linkages of the `missing_staff_id` sample dataset.
...
They either relax or strengthen the match criteria. Here is an example below.
```{r warning = FALSE}
...
This eventually becomes memory intensive.
Arguments such as `blocking_attribute`, `repeats_allowed`, `permutations_allowed ` and `data_source` are used to limit the creation and comparison of only certain types of record-pairs. 
...
There are other functions which support the above mentioned ones but are also useful for common analytical tasks.
...
str(prs_1); str(prs_2); str(prs_3)
```

## `combi()`
Numeric codes for unique combination of vectors.
This is a useful and faster alternative to `paste0` when it's more relevant to know a unique combination instead of the exact combination.
```{r warning=FALSE}
...
The flexibility of `episodes()` and `links()` results in situations where the use of different arguments will lead to the same outcome. However, one combination will usually require the least number of iterations to complete the same process. 
...
For some analyses, `episodes()` and `links()` can be used interchangeably however, this tends to be less efficient. 
...
These time differences are negligible when dealing with relatively small datasets but become more apparent as the size increases.
