---
title: "Overview: diyar at a glance"
date: "`r format(Sys.Date(), '%d %B %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Overview: diyar at a glance}
...
Linked records are assigned to unique groups represented by identifiers.
These identifiers are stored as an `S4` class with useful information about each group in its slots.

The package is capable of assessing and comparing a record's characteristics in several ways, making it useful in ordinarily complex data analysis such as record linkage and contact or network analysis. 
The main functions responsible for these analyses are `links()`, `episodes()` and `partitions()`.
These functions provide a lot of flexibility in terms of what and how comparisons are done, as well as what are considered matches.
Each is best suited to different use cases explained below.

  + `links()` - Comparing records without a temporal aspect to them.
  + `episodes()` - Comparing records with a temporal aspect to them, while factoring the duration between individual records.
  + `partitions()` - Comparing records with a temporal aspect to them, without factoring the duration between individual records.

It's worth noting that `links()` and `episodes()` are iterative (loops) functions. 
At their core, they are simply repeating comparisons of an index record against other records. 

# Record groupping functions
## `links()`
`links()` is primarily used for record linkage, particularly multi-stage record linkage.
Here, records are matched on different attributes in successive stages. 
The priority of matches decreases with each stage, that is; a match at one stage (and thus attribute) is considered more relevant than a match at the next stage.
As a result, changing the order of each stage/attribute results in a different linkage strategy, which in turn leads to different outcomes.
This is demonstrated below with the `missing_staff_id` sample dataset.
...
dfr_stages
```
User-defined comparisons are also possible using a `sub_criteria` object.
These are `S3` objects that contains attributes and the functions used to compare them.
They are used to either relax or strengthen the conditions for a match.
An example below.
```{r warning = FALSE}
...
```
`sub_criteria` can also be nested for nested conditions.
```{r warning = FALSE}
...
## `partitions()`
`partitions()` assigns events to groups (panes) if they are within a defined interval (Figure 1). Unlike `episodes()`, the duration between events is not a factor. Here, events from the same pane simply occurred within the same interval. See examples below.
...
## `link_records()`
Implements probabilistic record linkage.
Record-pairs are generated and compared for matches.
Matches can be weighted to allow for imperfect matches.
Match probabilities are then calculated and used to determine assign a match score.
This can be memory intensive as the number of record doubles exponentially therefore,
`links_wf_probabilistic()` - a wrapper function of `links()`, is provided to
compare batches of record-pairs in iterations.  
```{r warning=FALSE}
...
p6a <- link_records(dfr_5, cmp_threshold = 1,
                           cmp_func = c(exact_match, exact_match,
                                        last_word_cmp, last_word_cmp),
                           score_threshold = -4)
p6b <- links_wf_probabilistic(dfr_5, cmp_threshold = 1,
                              cmp_func = c(exact_match, exact_match,
                                           last_word_cmp, last_word_cmp),
                              score_threshold = -4,
                              recursive = TRUE)
...
# Weights
subset(as.data.frame(p6a$pid_weights), prb.threshold == 1)
subset(as.data.frame(p6b$pid_weights), prb.threshold == 1)
```


# Processing time
As mentioned, `episodes()` and `links()` are iterative functions.
Although both continue to be optimised, each iteration costs additional processing time.
The main way to mitigate this is to reduce the number of iterations required to complete an analyses. 
...
In addition, the flexibility of `episodes()` and `links()` results in situations where the use of different arguments lead to the same outcome. 
However, some approach will often require less iterations to complete the same process compared to others. 
...
Furthermore, for some analyses, `episodes()` and `links()` can be used interchangeably however, this tends to be less efficient. 
For example, `dfr_4$ep4` and `dfr_4$pd5` are essentially the same outcome; two groups with the same set of records in each. However, we can see below that `episodes()` created `dfr_4$ep4` in two iterations compared to the 7 iterations it took `links()` to create `dfr_4$ep4`.
```{r warning=FALSE}
summary(dfr_4$ep4)
...
For example, `dfr_stages$p4` was created using `sub_cri_b` which took `r max(dfr_stages$p4@iteration)`.
However, the same outcome can be achieved in 1 iteration with a `criteria`. 
See the difference below.
```{r warning=FALSE}
dfr_stages$p4b <- links(paste0(last_word_wf(dfr_stages$hair_colour),
...
These time difference are negligible when dealing with relatively small datasets but become more apparent as the size increases.
