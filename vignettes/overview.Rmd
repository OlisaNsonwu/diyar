...
These functions are very flexible in terms of what and how comparisons are done, as well as what are considered matches.
...
Arguments such as `blocking_attribute`, `repeats_allowed`, `permutations_allowed ` and `data_source` are used to limit the creation and comparison of only certain record-pairs. 
...
## combi
`combi()` creats numeric codes for unique combination of vectors.
...
The flexibility of `episodes()` and `links()` results in situations where the use of different arguments will lead to the same outcome. However, one combination will usually require fewer number of iterations to complete the same process. 
