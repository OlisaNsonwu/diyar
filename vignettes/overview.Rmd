...
The main functions are `links()` and `links_sv_probabilistic()`, `episodes()` and `partitions()`.
...
p6a <- links_sv_probabilistic(dfr_5, attr_threshold = 1,
...
# Weights
subset(p6a$pid_weights, as.logical(record.match))
```
...
Also primarily used for probabilistic record linkage. `links_wf_probabilistic()` is a wrapper function of `links()` and an alternative to `links_sv_probabilistic()`. It's less memory intensive but can be slower in comparison.
```{r warning=FALSE}
