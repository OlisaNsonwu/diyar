...
# Overview
The group identifiers created by `episodes()` and `partitions()` are called episodes (`epid` class) and panes (`pane` class) respectively. Additionally, `episodes()` creates three types of episodes - `"fixed"`, `"rolling"` and `"recursive"`. In `diyar`, a fixed episode is created by linking an index or reference event (dated records) to other events within a specified period of another event. This yields `"Case"` (index) and `"Duplicate"` events. In a rolling episode, the process is repeated using an event from the existing episode as the reference event. This yields a `"Recurrent"` event and additional `"Duplicate"` events. Here, this repetition is referred to here as a recurrence. Unless specified, the recurrence will continue indefinitely until there are no more events within the period of recurrence. A recursive episode is similar to a rolling episode except that every event in the existing episode is used as a reference event. In contrast, a pane is created by separating events into fixed periods of time. The events in a pane have no relationship with each other, other than being in the same period or numeric interval. See the figure and example below.
...
s_data$pn1 <- partitions(event_dt, length.out = 2, separate = TRUE,
                         group_stats = TRUE, data_source = attr_2)
...
# Implementation
The main considerations in a case definition are captured by different arguments in these functions.
...
**Figure 3: Using a `sub_criteria` to specify matching criteria**
...
```{r warning=FALSE}
# Group events into 2 equal parts based on the strata's duration
...
The `from_last` argument specifies the direction of episode tracking, while `custom_sort` specifies a custom preference for selecting index events. The combination of both provides some flexibility in regards to which event is used as the index event. See the examples below.
```{r warning=FALSE}
...
## Recurrence
The `episode_type` argument can be used to request for rolling or recursive episodes, which permit recurrence. `reference_event` is used to specify which of the events in the existing episode is used as the reference event for the next recurrence. `case_for_recurrence` determines if the initial occurrence of the event and subsequent recurrences are to be treated in the same way i.e. should recurrent events trigger a new initial occurrence of their own.
...
Multiple `case_length` or `recurrence_length` can be specified by providing a `list` of ranges (`number_line()`), while the `case_length_total` and `recurrence_length_total` arguments specifies how many such temporal links are required for an episode.
...
## Interval grouping
It's possible to track episodes among events with known start and end points instead of those occurring in single point in time. It is worth noting that the reference point for such events is the end point. Therefore, using a `case_length` of `1` when working such events is interpreted as 2 days (1-day difference) after the end point of the interval. By default, other events that occur on or after the start point but before end point of the index event will not be captured. If this is required, use `index_window()` to get the range required to capture such events, and supply it to the `case_length` or `recurrence_length` arguments. See the figure and examples below.
```{r warning=FALSE}
...
# Fast
ep23a <- episodes(duplicate_events, case_length = 5, episode_type = "recursive")
# Faster
ep23b <- episodes(duplicate_events, case_length = 5, episode_type = "rolling", 
                  reference_event = "last_event")
# Fastest
ep23c <- episodes(duplicate_events, case_length = 5, episode_type = "rolling", 
                  reference_event = "last_record")
```

```{r warning=FALSE, include=FALSE}
# plt1 <- schema(ep23a, seed = 2, show_label = c("length_label", "length_arrow"), dark_mode = FALSE)
# plt2 <- schema(ep23b, seed = 2, show_label = c("length_label", "length_arrow"), dark_mode = FALSE)
# plt3 <- schema(ep23c, seed = 2, show_label = c("length_label", "length_arrow"), dark_mode = FALSE)
# f <- plot_grid(plt1 + theme(plot.background = element_rect(color = "black")),
#                plt2 + theme(plot.background = element_rect(color = "black")),
#                plt3 + theme(plot.background = element_rect(color = "black")),
#                labels = c("Fast (ep23a)",
...
Reframing the dataset in such a way that you have to deal with fewer records can also save some processing time. For example, assume a dataset of 10 events from 5 individuals on 3 separate days and a case definition to assess a potential transmission event between these individuals. Instead of analysing 10 events (records) with the person identifier as a sub-criteria, change the dataset to 3 dates (records) with a `list` of all individuals per date. The `list` can then be passed to `case_sub_criteria` or `recurrence_sub_criteria` for the necessary logical test(s). `episodes_wf_splits()` is an wrapper function of `episodes()` designed to achieve this.

### Stop episode tracking
`episode_max`, `rolls_max` and `skip_order` are arguments used to stop the episode tracking process "prematurely" once certain conditions are met. By doing this, the total number of iterations and processing time is therefore reduced. `episodes_max` specifies the maximum number of episodes required from a `strata`. `rolls_max` specifies the number of recurrences an index case is allowed. `skip_order` stops the episode grouping process when all records with the `nth` level of a `custom_sort` have been linked to an episode. `skip_order` is particularly useful when tracking rare events among other common events. Once all rare events have been tracked, the process is stopped. The example below has a mix of two `"Rare"` events and 20 `"Common"` events, and takes 11 iterations to complete.
...
duplicate_events_2 <- rep(event_dt, 2)
ep24a <- episodes(duplicate_events_2, case_length = 0, custom_sort = attr_4, data_source = attr_4)
summary(ep24a)
```
...
```{r warning=FALSE}
ep24b <- episodes(duplicate_events_2, case_length = 0, custom_sort = attr_4, skip_order = 1, 
                  data_source = attr_4)
...
`episodes` can be tracked in other units of time (`episode_unit`). Acceptable options are `"seconds"`, `"minutes"`, `"hours"`, `"days"`, `"weeks"`, `"months"` or `"years"`. Below is an example of tracking `episodes` by the hour compared tracking them by the day (default).
...
`numeric` values can be supplied to the `date` argument. When using the function this way, the `episode_unit` argument is ignored.
...
You can incorporate missing and infinite cut-offs into your analyses. Events with missing a cut-off will not be used as an index event however, they can be tagged as a duplicate of another index event. Index events with an infinite cut-off will be linked to every other event in its `strata`.
