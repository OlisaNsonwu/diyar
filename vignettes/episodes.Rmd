---
title: "Introduction to epidemiological case definitions with `episodes`"
date: "`r format(Sys.Date(), '%d %B %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to epidemiological case definitions with `episodes`}
...
The simplest kind of case definitions are those where only only event is retained making every other event a duplicate `(D)` i.e. the same occurrence of the retained event (index) event `(C)`.
```{r warning=FALSE}
...
`episodes()` can also produces two additional type of episodes - `"rolling"` and `"recursive"`. See examples of each below.
```{r warning=FALSE}
...
These are recurrence of index events as opposed to their duplicates `(D)`. For example, `ep3` initially ended on `2021-01-06`. However, for rolling and recursive episodes, the function takes the additional step of looking for other events within 4 days of `2021-01-06`. Records within this period are then categorised as recurrent events of the `2021-01-06` event, which itself is a duplicate of the `2021-01-01` event. This chaining process continues indefinitely until there are no more events within the period of recurrence or it's stop by the `rolls_max` argument.

Even though `ep1` has the same outcome (one episode) as `ep3` and `ep4`, they are very different case definitions.
...
event_dt
```
As seen above, the last record is now the index of the episode.
...
This determines which record is used as the reference when checking for other events within it's window of occurrence.
...
event_dt$ep6 <- episodes(event_dt$date, 3, episode_type = "rolling", 
                         custom_sort = custom_pref, skip_order = 1)
event_dt$ep7 <- episodes(event_dt$date, 3, episode_type = "rolling", 
                         custom_sort = custom_pref, skip_order = 1, from_last = TRUE)
```
...
Another main aspect of the tracking process is the duration between event.
This is handled by the `case_length` and `recurrence_length` arguments.
Unless specified, `recurrence_length` defaults to the `case_length`.
...
schema(dfr$ep1, seed = 2, show_labels = FALSE)
schema(dfr$ep2, seed = 2, show_labels = FALSE)
```
...
Alternatively, we can specify that BSI events or those that occurred in Ward 1 should not be linked to any episode.
```{r warning=FALSE}
...
event_dt$ep12 <- episodes(event_dt$date, case_length = 5, case_sub_criteria = combined_sub_cri)
```
