...
***NOTE. `to_s4` and `to_s4()` changes the function's output from a data.frame (current default) to `pid` objects. `pid` objects will be made the default output in the next major release.***
...
Opes_c$pids_a <- record_group(Opes_c, criteria = dummy_cri, sub_criteria = list(s1="range_a"), to_s4 =TRUE)
...
Opes_c$pids_b <- record_group(Opes_c, criteria = dummy_cri, sub_criteria = list(s1="range_b"), to_s4 =TRUE)

Opes_c[c("age","range_b","pids_b")]
```

Only use `number_line` objects as a `sub_criteria`. **Do not directly use `number_line` objects as a `criteria`!** Instead, create a dummy `criteria` (e.g. `1` for every row), and then use the `number_line` object as a `sub_criteria` paired to the dummy `criteria`.
