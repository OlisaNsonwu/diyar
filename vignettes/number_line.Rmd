...
You can also convert `number_line` objectss to a sequence of `numeric` based values
...
You can choose to only compress `number_line` objects that overlap in a particular way using the `methods` arguement.
...
The resulting `number_line` objects from a compressed pair can yet again overlap with others, especially when not all `methods` were used. By default, resulting compressed `number_line` objects are not compressed a second time however, you can do so using the `collapse` arguement. It's worth mentioning that this process starts from the 1^st^ `number_line` object and then proceedes to the last in the set. Therefore, their position in the set can lead to different results. 
...
As mentioned, if each `number_line` object's postion in the set changes, the result could also change. See below. 
...
However, you can use the `collpase` arguement to further compress already compressed `number_line` objects if they overlap again. In the example below, we collapse (rather than compress) the inital set (`nl_zb`) which gives the same result as compressing the rearranged version. 
