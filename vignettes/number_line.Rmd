...
# Introduction
A `number_line` object is a range of `numeric` values on a number line. `interval` objects (`lubridate` package) are the inspiration for `number_line` objects. `number_line` objects are similar but support any `numeric` based object class.
...
For an `"increasing"` `number_line` object, the `start_point()` is on the `left_point()`, while for a `"decreasing"` `number_line` object, the `start_point()` is on the `right_point()`.
...
There are convenience functions to reverse, shrink, expand objects, or shift a `number_line` along the number line.
...
# Shift the `number_line` towards the left of the number line by 2 days 
nl_i <- shift_number_line(nl_c, -2); nl_i

# Shift the `number_line` towards the right of the number line by 2 days 
nl_j <- shift_number_line(nl_c, 2); nl_j

# Change the left part of the `number_line` from to a negative value
...
## Overlaps
`number_line` objects can overlap with each other.  They do so in seven ways - `exact()`, `reverse()`, `inbetween()`, `"across()`, `chain()`, `aligns_start()` and `aligns_end()`. The 7 methods are mutually exclusive. There are 2 convenience methods - `"none"` and `overlap()`., which are mutually inclusive with the other 7. See Figure 1.
...
```{r warning=FALSE, include=FALSE}
 ...
 #                label_colour = "black")
 # ggsave(plot = f, filename = "fig_n1.png", width = 15, height = 9, units = "in")
```
