...
However, you can use the `collapse` argument to further compress already compressed `number_line` objects if they overlap again. In the example below, we collapse (rather than compress) the initial set (`nl_zb`) which gives the same result as compressing the rearranged version. 
