...
# Introduction
A `number_line` object is a range of `numeric` values. `interval` objects (`lubridate` package) are the inspiration for `number_line` objects as they are similar but `number_line` supports any `numeric` based object class.
...
For an `"increasing"` `number_line`, the `start_point()` is at the `left_point()`. In contrast, the `start_point()`  of a `"decreasing"` `number_line` is at the `right_point()`.
