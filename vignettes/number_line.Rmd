...
`diyar` has convenience functions to reverse, shrink or expand `number_line` objects, or shift them along the number line. 
...
The resulting compressed `number_line` object from an overlapping pair can yet again overlap with others, especially when not all `methods` were used. By default, the resulting compressed `number_line` object is not compressed a second time however, you can do so by using the `collapse` argument. It's worth mentioning that this process starts from the first `number_line` object in the set and then proceeds to the last. Therefore, their position in the set can lead to different results. 
...
In the figure above, there are `"across"`, `"chain"`, `"inbetween"` and `"aligns_end"` overlaps but chose to only compress pairs that overlap by `"chain"` and `"aligns_end"` methods. The resulting `number_line` object from compressing ``r paste0(format(nl_z[1:2]), collapse=" and ")`` now overlaps with the resulting `number_line` objects from ``r  paste0(format(nl_z[3:4]), collapse=" and ")``.
