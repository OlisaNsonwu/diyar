...
`diyar` has convenience functions to reverse, shrink, expand `number_line` objects, or shift thm along the number line. 
...
You can compress a pair of overlapping `number_line` objects into one by using `compress_number_line()`. You can also choose to compress only those that overlap in a certain way among the 6 methods shown above.

When the overlapping `number_line` objects have different directions, the resulting compressed `number_line` object will inherit the direction of the widest among them. If they have the same width, the compressed `number_line` object takes on an `"increasing"` direction.
...
The resulting compressed `number_line` object can yet again overlap with others, especially when not all `methods` were used. By default, the resulting compressed `number_line` object is not compressed a second time however, you can do so by using the `collapse` argument. 
...
**Figure 8a: Compressing pairs of `number_line` objects by `"across"`, `"chain"`, `"inbetween"` and `"aligns_end"` methods**
```{r message=FALSE, warning=FALSE}
...
**Figure 8b: Compressing pairs of `number_line` objects by `"chain"` and `"aligns_end"` methods compressed into one**
...
**Figure 8c: Rearranged the pairs of `number_line` objects before compressing by `"across"`, `"chain"`, `"inbetween"` and `"aligns_end"` methods **
...
You can use the `collapse` argument to further compress already compressed `number_line` objects if they overlap again. In the example below, we collapse (rather than compress) the initial set (`nl_z`) which gives the same result as compressing the rearranged version (`nl_zb`). 
