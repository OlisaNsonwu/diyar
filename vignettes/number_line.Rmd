...
# 
# exact_sort <- rep(c("x", "y"), length(exact)/2)
# inbetween_sort <- rep(c("x", "y"), length(inbetween)/2)
# across_sort <- rep(c("x", "y"), length(across)/2)
# chain_sort <- rep(c("x", "y"), length(chain)/2)
# aligns_start_sort <- rep(c("x", "y"), length(aligns_start)/2)
# aligns_end_sort <- rep(c("x", "y"), length(aligns_end)/2)
# 
# nl2 <- c(exact, inbetween, across, chain, aligns_start, aligns_end)
# strata <- sort(rep(seq_len(length(nl2)/2), 2))
# c_sort <- c(exact_sort, inbetween_sort, across_sort, chain_sort, aligns_start_sort, aligns_end_sort)
# nl_label <- paste0(ifelse(nl_ep@sn == nl_ep@.Data,"y: ", "x: "), " (", format(nl2), ")")
# nl_label <- paste0(ifelse(nl_ep@sn == nl_ep@.Data,"y: ", "x: "))
# nl_ep <- episodes(nl2, strata = strata, case_length = index_window(nl2), custom_sort = c_sort)
# plt1 <- schema(nl_ep[1:12], seed = 2, show_labels = c("case_overlap_methods", "date"),
#                theme = "light", custom_label = nl_label[1:12])
# plt2 <- schema(nl_ep[13:20], seed = 2, show_labels = c("case_overlap_methods", "date"),
#                theme = "light", custom_label = nl_label[13:20])
