...
htest_ep <- diyar::episodes(date = number_line(epids_htest_1$period_start, epids_htest_1$period_end),
                           ...
                           group_stats = T,
                           case_overlap_methods = epids_htest_1$overlap_methods_cd,
                           skip_if_b4_lengths = F,
                           display = "progress",
                           sn = epids_htest_1$sn)

all(epids_htest_1$epid == htest_ep@.Data)
all(epids_htest_1$wind_nm == decode(htest_ep@wind_nm))
...
mtest_ep <- episodes(sn = epids_mtest_1$sn,
                     strata = epids_mtest_1$strata,
                     date = epids_mtest_1$date,
                     case_length = list(epids_mtest_1$case_length,
...
all(epids_mtest_1$wind_nm == decode(mtest_ep@wind_nm))
