...
+ New function - `attrs()`. Used to pass set of attributes to an instance of `match_funcs` or `equal_funcs`.
+ New `S3`object - `d_report`. Status of the analysis over its run time. Use 
+ New `S3`object - `d_attribute`. XXXXXXXX

## Changes
+ Optimised `episodes()` and `links()`. Reduced processing times.
+ Three new options for the `display` argument. `"progress_with_report"`, `"stats_with_report"` and `"none_with_report"`.
+ `eval_sub_criteria()`. Record-pairs are no longer created in the function. Therefore, the `index_record` and `sn` have been replaced with `x_pos` and `y_pos`.

## Bug fixes
+ Incorrect `wind_id` list in `episodes()` when `data_link` is used. Resolved.
