...
## Changes
+ `sub_criteria()` - Output of `match_func` and `equal_func` can now be any atomic object.
+ `links()` - When `shrink` is `TRUE`, records in a record-group must meet every listed match `criteria` and `sub_criteria`. For example, if `pid_cri` is 3, then the record must have meet
...
+ `match_func` and `equal_func` of `sub_criteria` can now return a `list` where the first element is a logical object. Subsequent elements of the `list` are exported by `eval_sub_criteria()`
...
+ `make_ids()` - Does not capture all records in that should be in a record-group when matches are recursive. Resolved.
