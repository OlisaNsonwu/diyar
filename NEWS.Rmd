...
+ New function - `true()`. Predefined logical tests for use with `sub_criteria()`.
+ New function - `false()`. Predefined logical tests for use with `sub_criteria()`.
...
+ New argument in `sub_criteria()`- `nested`. XXXXXX.

## Changes
+ `sub_criteria` - Output of `match_func` and `equal_func` can now be any atomic object.
+ `links` - When `shrink` is `TRUE`, records in a record-group must meet every listed match `criteria` and `sub_criteria`.

## Bug fixes
+ `episodes()` - Incorrect results in some instances with `skip_order`. Resolved.
+ `make_ids()` - Does not capture the complete record-group when matches are recursive. Resolved.
