...
+ Episode-level options for `episode_type`, `skip_if_b4_lengths`, `case_for_recurrence` and `recurrence_from_last`. 
The length of these arguments can now be the same as `date`. This allows for simultaneous tracking of episodes in different ways within the same instance of `episodes()`. 
The behaviour for each episode is determined by the corresponding option for the episode's index event (`"Case"`).
