...
episodes_wf_splits <- function(..., to_s4 = T){
  if(to_s4 == F){
    as.data.frame(diyar::episodes_wf_splits(..., display = "none"), stringsAsFactors = FALSE)
  }else{
...
test_1 <- cbind(T1,
                episodes_wf_splits(strata = paste0(T1$pid, " ", c(rep("DS-A", 10), rep("DS-B", 10))),
                         date = T1$date,
...
test_2 <- cbind(data_2,
                episodes_wf_splits(strata = paste0(data_2$pid, " ", c(rep("DS-A", 10), rep("DS-B", 10))),
                         date = data_2$date,
...
test_6 <- cbind(test_6,
                episodes_wf_splits(strata = paste0(test_6$pid, " DS-", sort(rep(c("A","B","C"), 10))),
                         date = test_6$date,
...
test_8a <- cbind(hospital_infections,
                 episodes_wf_splits(sn= hospital_infections$rd_id, date = hospital_infections$date, case_length = hospital_infections$epi_len,
...
test_9b <- cbind(hospital_infections,
                 suffix(episodes_wf_splits(sn = hospital_infections$rd_id, date = hospital_infections$date,
...
test_10a <- cbind(hospital_infections,
                  episodes_wf_splits(sn = hospital_infections$rd_id, date = hospital_infections$date, strata = hospital_infections$patient_id,
...
test_10b <- cbind(hospital_infections,
                  episodes_wf_splits(sn = hospital_infections$rd_id, date = hospital_infections$date,
...
test_11a <- cbind(
  admissions,
  episodes_wf_splits(date = admissions$admin_period, sn=admissions$rd_id,
...
epids <- episodes_wf_splits(date = date(c(x1, x2, x3, x4, x5)),
                  strata = strt,
                  case_length = c,
                  recurrence_length = r,
                  to_s4=T,
...
e_case_nm <- c("Case","Duplicate_C","Recurrent","Duplicate_R","Duplicate_R",
               "Case","Duplicate_C","Recurrent","Case","Duplicate_C",
               "Case","Duplicate_C","Recurrent","Duplicate_R","Duplicate_R","Duplicate_R",
               "Case","Duplicate_C","Recurrent","Duplicate_R","Case","Duplicate_C",
               "Case","Duplicate_C","Recurrent","Duplicate_R","Recurrent","Duplicate_R",
               "Case","Duplicate_C","Case","Duplicate_C","Duplicate_C","Recurrent",
               "Case","Recurrent","Duplicate_R","Recurrent",
               "Case","Recurrent","Duplicate_R","Case",
               "Case","Recurrent","Duplicate_R","Duplicate_R","Recurrent","Duplicate_R",
               "Case","Recurrent","Duplicate_R","Duplicate_R","Recurrent","Case",
               "Case","Recurrent","Duplicate_R","Duplicate_R","Case","Duplicate_C")
...
epids_r <- episodes_wf_splits(date = df$x, case_length = df$c,  recurrence_length = df$r, to_s4 = T, episode_type = "rolling")

x <- c(date("01/01/2007"), date("10/01/2007"), date("12/01/2007"), date("15/01/2007"), date("22/01/2007"))
df <- data.frame(x=x, c=5, r=10)
epids2_r <- episodes_wf_splits(date = df$x, case_length = df$c,  recurrence_length = df$r, to_s4 = T, episode_type = "rolling")
...
df$r1 <- episodes_wf_splits(d, case_length = 3, case_for_recurrence = F, to_s4 = T, episode_type = "rolling")
df$r2 <- episodes_wf_splits(d, case_length = 3, case_for_recurrence = T, to_s4 = T, episode_type = "rolling")
...
df$c5 = c5
df$c7 = c7
df$ep1 <- episodes_wf_splits(date = df$dt, case_length = df$c1)
df$ep2 <- episodes_wf_splits(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)))
# Test - Expect TRUE
all(df$ep1==df$ep2)

df$ep3 <- episodes_wf_splits(date = df$dt, case_length = list(df$c2, index_window(df$dt)))
df$ep4 <- episodes_wf_splits(date = df$dt, case_length = list(df$c2, invert_number_line(df$c2), index_window(df$dt)))
# Test - Expect TRUE
all(df$ep3==df$ep4)

df$ep5 <- episodes_wf_splits(date = df$dt, case_length = df$c3)
df$ep6 <- episodes_wf_splits(date = df$dt, case_length = list(df$c3, invert_number_line(df$c3), index_window(df$dt)))
# Test - Expect TRUE
all(df$ep3==df$ep6)

df$ep7 <- episodes_wf_splits(date = df$dt, case_length = df$c3, custom_sort = df$c_sort)

df$ep8 <- episodes_wf_splits(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)), custom_sort = df$c_sort)
df$ep9 <- episodes_wf_splits(date = df$dt, case_length = list(df$c5, invert_number_line(df$c5)), custom_sort = df$c_sort)
# Test - Expect TRUE
all(df$ep8 == df$ep9)

#test that these two should give the same result
df$ep10 <- episodes_wf_splits(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)), custom_sort = df$c_sort, from_last = T)
df$ep11 <- episodes_wf_splits(date = df$dt, case_length = list(df$c5, invert_number_line(df$c5)), custom_sort = df$c_sort, from_last = T)
...
df$ep1 <- episodes_wf_splits(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = T, episode_type = "rolling", rolls_max = 1)
df$ep2 <- episodes_wf_splits(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, episode_type = "rolling", rolls_max = 1)

df$ep3 <- episodes_wf_splits(date = df$x, case_length = list(df$ep, 0), recurrence_length = list(df$rc, 0), case_for_recurrence = T, reference_event = "first_record", episode_type = "rolling", rolls_max = 1)
df$ep4 <- episodes_wf_splits(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, reference_event = "first_record", episode_type = "rolling", rolls_max = 1)

df$ep5 <- episodes_wf_splits(date = df$x, case_length = list(df$ep, 0), recurrence_length = list(df$rc, 0), case_for_recurrence = T, episode_type = "rolling", rolls_max = 2)
df$ep6 <- episodes_wf_splits(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, episode_type = "rolling", rolls_max = 2)
...
df1$ep1 <- episodes_wf_splits(date = df1$x, case_length = df1$cl, recurrence_length = df1$rc, episode_type = "rolling", skip_if_b4_lengths = T)
df1$ep2 <- episodes_wf_splits(date = df1$x, case_length = df1$cl, recurrence_length = df1$rc, episode_type = "rolling", skip_if_b4_lengths = F)

x <- seq(1,20,3)
df2 <- data.frame(x=x, rc=0)
df2$ep <- number_line(6,6)
df2$ep3 <- episodes_wf_splits(date = df2$x, case_length = df2$ep, recurrence_length = df2$rc, episode_type = "rolling", skip_if_b4_lengths = T)
df2$ep4 <- episodes_wf_splits(date = df2$x, case_length = df2$ep, recurrence_length = df2$rc, episode_type = "rolling", skip_if_b4_lengths = F)
...
ds$c4 <- 0
ds$ep_1 <- episodes_wf_splits(date = ds$x, case_length = ds$c1, skip_if_b4_lengths = T)
ds$ep_2 <- episodes_wf_splits(date = ds$x, case_length = ds$c1, custom_sort = ds$s)
ds$ep_3 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_4 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, skip_if_b4_lengths = T)
ds$ep_5 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_6 <- episodes_wf_splits(date = ds$x, case_length = ds$c4, recurrence_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_7 <- episodes_wf_splits(date = ds$x, case_length = ds$c4, recurrence_length = ds$c3, episode_type = "rolling")
ds$ep_8 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, recurrence_length = ds$c3, episode_type = "rolling", skip_if_b4_lengths = T)


ds$c5 <- number_line(-7,-10)
ds$c6 <- number_line(-10,-0)
ds$c7 <- number_line(-10, 10)
ds$ep_9 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, custom_sort = ds$s, from_last = T)
ds$ep_10 <- episodes_wf_splits(date = ds$rx, case_length = ds$c2, custom_sort = ds$s)
ds$ep_11 <- episodes_wf_splits(date = ds$x, case_length = ds$c5, custom_sort = ds$s, from_last = T)
ds$ep_11.2 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_12 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_13 <- episodes_wf_splits(date = ds$x, case_length = ds$c6, custom_sort = ds$s)

ds$ep_14 <- episodes_wf_splits(date = ds$x, case_length = ds$c7, custom_sort = ds$s)
ds$ep_15 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c7, invert_number_line(ds$c7)), custom_sort = ds$s)
ds$ep_16 <- episodes_wf_splits(date = ds$x, case_length = ds$c7, custom_sort = ds$s, from_last = T)
...
ds$c3 <- number_line(0,10)
ds$c4 <- 0
ds$ep_1 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c1, index_window(ds$x)))
ds$ep_2 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c1, index_window(ds$x)), custom_sort = ds$s)
ds$ep_3 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_3.5 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_4 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), skip_if_b4_lengths = T)
ds$ep_5 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_5.5 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_6 <- episodes_wf_splits(date =ds$x, case_length = list(ds$c4, index_window(ds$x)), recurrence_length = list(ds$c2, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_7 <- episodes_wf_splits(date =ds$x, case_length = list(ds$c4, index_window(ds$x)), recurrence_length = list(ds$c3, index_window(ds$x)), episode_type = "rolling")
ds$ep_8 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), recurrence_length = list(ds$c3, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)

ds$c5 <- number_line(-7,-10)
ds$c6 <- number_line(-10,-0)
ds$c7 <- number_line(-10, 10)

ds$ep_9 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x, from_last = T)), custom_sort = ds$s, from_last = T, skip_if_b4_lengths = T)
ds$ep_9.2 <- episodes_wf_splits(date = ds$x, case_length = ds$c2, custom_sort = ds$s, from_last = T, skip_if_b4_lengths = T)

ds$ep_11 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s, from_last = T)
ds$ep_11.2 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_12 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s)
ds$ep_13 <- episodes_wf_splits(date = ds$x, case_length = list(ds$c6, index_window(ds$x)), custom_sort = ds$s)
